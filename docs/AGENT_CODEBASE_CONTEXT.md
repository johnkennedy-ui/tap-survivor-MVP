...
- Module bridge build: `scripts/build-module-bridges.mjs`; currently generates classic bridges from `src/modules/`.
  Covered bridge entries include audio, balance, level-up-choices, map-system, math, save-corruption, save-defaults,
...
- Runtime dependency ownership: `src/modules/game-dependencies.js` owns dependency collection from retained classic globals,
  constructs source-owned audio from explicit `globalRef` Audio, AudioContext, and clock factories, and supplies
  `bindMovementInput` from `src/modules/input.js`; the classic `src/game.js` fallback boundary receives them through
  `TapSurvivorGameDependencies` without `TapSurvivorAudio` or `TapSurvivorInput` lookups.
