...
- Module bridge build: `scripts/build-module-bridges.mjs`; currently generates math, shop-pricing, weapon-cooldowns, and weapon-targeting classic bridges from `src/modules/`.
...
- Weapon firing behavior: `src/weapon-fire.js` remains the public integration entry point.
  Cooldown/stat scaling ownership lives in `src/modules/weapon-cooldowns.js`, projectile behavior in
