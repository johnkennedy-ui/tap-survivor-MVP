...
- Module bridge build: `scripts/build-module-bridges.mjs`; currently generates classic bridges from `src/modules/`.
  Covered bridge entries include balance, level-up-choices, map-system, math, save-corruption, save-defaults, shop-pricing, weapon-cooldowns, weapon-projectiles, and weapon-targeting.
...
- Save helper ownership: defaults live in `src/modules/save-defaults.js`, migrations in `src/save-migrations.js`, normalization in `src/save-normalize.js`, and corrupt-load warning behavior in `src/modules/save-corruption.js`.
  `src/save-defaults.js` and `src/save-corruption.js` are generated by `npm run build:bridges`.
