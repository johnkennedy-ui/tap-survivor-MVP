...
- Module bridge build: `scripts/build-module-bridges.mjs`; currently generates classic bridges from `src/modules/`.
  ...
  game-runtime, game-dependencies, and run-lifecycle.
- Runtime dependency ownership: `src/modules/game-dependencies.js` owns dependency collection from classic globals, including `TapSurvivorInput.bindMovementInput`.
...
- Runtime controller ownership: `src/modules/game-runtime.js` owns boot/speed/reset/lifecycle behavior and receives movement input binding as an injected option; `src/game-runtime.js` is generated by `npm run build:bridges`.
