...
- Runtime dependency ownership: `src/modules/game-dependencies.js` owns dependency collection from retained classic globals,
  ...
  `bindMovementInput` from `src/modules/input.js`; the classic `src/game.js` fallback boundary imports that source-owned
  factory directly without `TapSurvivorGameDependencies`, `TapSurvivorAudio`, or `TapSurvivorInput` lookups.
