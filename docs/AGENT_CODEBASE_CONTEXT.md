...
- `src/modules/game-dependencies.js`: real ES-module implementation for collecting the runtime dependency bag consumed by `src/game.js`.
- `src/game-dependencies.js`: generated classic compatibility bridge for `TapSurvivorGameDependencies`; do not hand-edit it.
- `src/modules/game-runtime.js`: real ES-module implementation for runtime boot, speed controls, input binding, save reset, and lifecycle flush.
- `src/game-runtime.js`: generated classic compatibility bridge for `TapSurvivorGameRuntime`; do not hand-edit it.
...
- Module bridge build: `scripts/build-module-bridges.mjs`; currently generates classic bridges from `src/modules/`.
  Covered bridge entries include balance, level-up-choices, map-system, math, save-corruption, save-defaults,
  save-migrations, save-normalize, save, shop-pricing, weapon-cooldowns, weapon-projectiles, weapon-targeting,
  game-runtime, and game-dependencies.
- Runtime dependency ownership: `src/modules/game-dependencies.js` owns dependency collection from classic globals.
  `src/game-dependencies.js` is generated by `npm run build:bridges` and loaded immediately before `src/game.js`.
  Keep `src/game.js` as the side-effectful classic composition entrypoint until a later runtime-entry migration.
- Runtime controller ownership: `src/modules/game-runtime.js` owns boot/speed/reset/lifecycle behavior; `src/game-runtime.js` is generated by `npm run build:bridges`.
  Keep `src/game.js` on `globalThis.TapSurvivorGameRuntime.createGameRuntimeController` until a later runtime-entry migration removes the classic script-order contract.
- Quest graph audit: `scripts/audit-quests.mjs`.
- Save/meta progression: `src/modules/save.js`, `src/progression.js`, `src/run-state.js`, and orchestration in `src/game.js`.
