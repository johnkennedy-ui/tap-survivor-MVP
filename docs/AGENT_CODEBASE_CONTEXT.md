...
- Enemy and boss behavior: `src/combat.js`.
...
- Run-upgrade one-shot effects are interpreted in `src/upgrades.js`; combat-scaling run upgrades are read by tier ID in `src/combat.js`.
