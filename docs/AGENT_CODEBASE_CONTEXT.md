...
- Module bridge build: `scripts/build-module-bridges.mjs`; currently generates classic bridges from `src/modules/`.
  Covered bridge entries include balance, level-up-choices, map-system, math, save-corruption, save-defaults, save-migrations, save-normalize, save, shop-pricing, weapon-cooldowns, weapon-projectiles, and weapon-targeting.
