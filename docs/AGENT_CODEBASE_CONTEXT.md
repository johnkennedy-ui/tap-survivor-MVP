...
- Module bridge build: `scripts/build-module-bridges.mjs`; currently generates classic bridges from `src/modules/`.
  Covered bridge entries include balance, level-up-choices, map-system, math, save-corruption, shop-pricing, weapon-cooldowns, weapon-projectiles, and weapon-targeting.
...
- Save helper ownership: defaults live in `src/save-defaults.js`, migrations in `src/save-migrations.js`, normalization in `src/save-normalize.js`, and corrupt-load warning behavior in `src/modules/save-corruption.js`.
  `src/save-corruption.js` is generated by `npm run build:bridges`; keep consumers on the classic global bridge for now, and do not change defaults, migration, normalization, corrupt-save backup, or storage behavior when moving this bridge.
