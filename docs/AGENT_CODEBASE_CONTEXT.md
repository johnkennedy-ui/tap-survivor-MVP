...
- Module style: production starts at `src/app/production-module-autoboot.js` and passes the browser
  root once as an explicit `globalRef` capability. Downstream runtime composition uses ESM imports and
  dependency injection; retired `TapSurvivor*` publishers must not be restored or consumed.
...
- `src/game.js`: retained composition boundary, run loop, and high-level orchestration. Its browser
  root acquisition is limited to injecting platform capabilities into source-owned factories.
...
- Legacy script-order check: `scripts/check-script-order.mjs`.
- Module bridge build: `scripts/build-module-bridges.mjs`; it generates global-free compatibility artifacts from `src/modules/`.
...
- Runtime dependency ownership: `src/modules/game-dependencies.js` owns dependency collection from the retained raw
  ...
  `src/modules/input.js`; the retained `src/game.js` composition boundary imports that source-owned factory directly without
  ...
  `src/game.js` is a side-effectful composition boundary only: it must pass explicit capabilities and never restore a publisher lookup.
...
- Run lifecycle ownership: `src/modules/run-lifecycle.js` owns start/end/boss-clear behavior; `src/run-lifecycle.js` is generated by `npm run build:bridges`. The dependency bag injects the native factory into composition.
- Run state ownership: `src/modules/run-state.js` owns run state/player reset construction; `src/run-state.js` is generated by `npm run build:bridges`. The dependency bag injects the native factory into composition.
- Run update ownership: `src/modules/run-update.js` owns run ticking/player movement/XP updates; `src/run-update.js` is generated by `npm run build:bridges`. The dependency bag injects the native factory into composition.
...
- Save helper ownership: defaults live in `src/modules/save-defaults.js`, migrations in `src/modules/save-migrations.js`, and normalization lives in `src/modules/save-normalize.js`.
  ...
  The dependency bag passes `createSaveSystem` and its helper capabilities explicitly into composition.
  Do not hand-edit generated save artifacts or add a browser-publisher fallback.
...
- Combat damage ownership: `src/modules/combat-damage.js` owns damageEnemy, damagePlayer, and reapEnemies behavior;
  `src/combat-damage.js` is generated by `npm run build:bridges`. Composition receives the native factory through its
  dependency bag.
- Coin shop: `src/shop.js` plus `shopItems` content and the in-run shop tab in `src/shell-ui.js`.
- Floor difficulty ownership: `src/modules/balance.js` owns the implementation; `src/balance.js` is generated by
  `npm run build:bridges`. Consumers receive it through explicit dependency wiring; preserve floor difficulty values
  and scaling when moving the seam.
- Level-up choice ownership: `src/modules/level-up-choices.js` owns the choice helper implementation;
  `src/level-up-choices.js` is generated by `npm run build:bridges`. Consumers receive it through explicit dependency
  wiring; preserve choice weighting, randomness, relic focus, unlocks, and level-up UI behavior when moving the seam.
- Map system ownership: `src/modules/map-system.js` owns the map/floor resolver implementation; `src/map-system.js` is
  generated by `npm run build:bridges`. Consumers receive it through explicit dependency wiring; preserve map/floor
  selection, background resolution, modifier merge, and floor-pool behavior when moving the seam.
- Math/helper ownership: `src/modules/math.js` owns the implementation; `src/math.js` is a generated global-free compatibility artifact.
- Shop pricing ownership: `src/modules/shop-pricing.js` owns the implementation; `src/shop-pricing.js` is a generated global-free compatibility artifact.
- Weapon cooldown ownership: `src/modules/weapon-cooldowns.js` owns the implementation; `src/weapon-cooldowns.js` is
  generated by `npm run build:bridges`. Keep cooldown, damage, reach, radius, projectile scaling, and SFX timing
  unchanged when moving the seam.
- Weapon projectile ownership: `src/modules/weapon-projectiles.js` owns the implementation;
  `src/weapon-projectiles.js` is generated by `npm run build:bridges`. Keep projectile firing, spread, bounce, pierce,
  explosion, split, damage, and collision behavior unchanged when moving the seam.
- Weapon targeting ownership: `src/modules/weapon-targeting.js` owns the implementation; `src/weapon-targeting.js` is a generated global-free compatibility artifact.
...
- For ES-module migration slices, put the real implementation in `src/modules/` and generate a global-free compatibility
  artifact with `npm run build:bridges`.
- Do not hand-edit generated bridge files such as `src/shop-pricing.js`. Generated artifacts preserve compatibility
  provenance only; new runtime dependencies must be explicit imports or dependency-bag capabilities, never publisher
  contracts.
