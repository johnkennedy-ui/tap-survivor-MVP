...
- Runtime dependency ownership: `src/modules/game-dependencies.js` owns dependency collection from retained classic globals
  and supplies `bindMovementInput` from `src/modules/input.js`; the classic `src/game.js` fallback boundary receives it
  through `TapSurvivorGameDependencies` without a `TapSurvivorInput` lookup.
