...
- `src/weapon-cooldowns.js`: weapon cooldown, SFX timing, damage, reach, width, projectile radius, and projectile skill scaling helpers.
- `src/weapon-projectiles.js`: projectile vector helpers plus projectile weapon firing, projectile spawning, bolt updates, bounces, split-on-hit, and explosion behavior.
...
- Module bridge build: `scripts/build-module-bridges.mjs`; currently generates math, shop-pricing, and weapon-targeting classic bridges from `src/modules/`.
...
- Weapon firing behavior: `src/weapon-fire.js` remains the public integration entry point.
  ...
  `src/weapon-projectiles.js`, targeting ownership in `src/modules/weapon-targeting.js`, and non-projectile
