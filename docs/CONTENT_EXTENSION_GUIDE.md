...
2. Edit `src/upgrades.js`.
3. For weapon damage upgrades, prefer the existing `createUpgradeDefs(weaponDefs)` pattern.
4. For in-run upgrades, add to `runUpgradeDefs`.
5. Use stable IDs and avoid duplicating an existing stat modifier.
6. Run `node --check src/upgrades.js`.
7. Run `npm test` if the upgrade can affect gameplay.

## Add An Item

1. Add the item to `shopItems` in `content/tap-survivor-content.json`.
2. Include `id`, `name`, `description`, `kind`, `cost`, and optional `effect`/`value`.
3. Do not wire new item behavior into `src/game.js` unless the task explicitly asks for it.
