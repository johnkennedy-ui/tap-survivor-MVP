...
check("level-up skill effects use projectile animation only", levelUp.includes("createChoiceIcon") && levelUp.includes("renderChoiceSprite") && levelUp.includes("trimmedFrameBounds") && levelUp.includes("animatedIcon === true") && Object.entries(content.weapons || {}).every(([id, weapon]) => content.assets?.sprites?.weapons?.[id]?.animatedIcon === (weapon.kind === "projectile")) && (content.runUpgrades || []).every((upgrade) => content.assets?.sprites?.runUpgrades?.[upgrade.id]?.animatedIcon === false));
