...
check("clean icons render on left HUD", sprites.includes("weaponIcon:") && sprites.includes("runUpgradeIcons") && renderHud.includes("weaponIcon:") && renderHud.includes("drawUpgradeRail") && renderHud.includes("runUpgradeIcon:") && (content.runUpgrades || []).every((upgrade) => content.assets?.sprites?.runUpgradeIcons?.[upgrade.id]?.includes("assets/generated/tower/sprites/")));
...
check("relic inventory menu exists", index.includes('id="menuInventoryTab"') && index.includes('id="menuRelicInventory"') && ui.includes("menuRelicInventory") && shellUi.includes("renderInventory"));
check("relic slots unlock by tower floor", save.includes("towerFloor: 1") && !save.includes("maxPlayerLevel") && !runUpdate.includes("recordPlayerLevel") && relics.includes("maxEquippedRelics") && relics.includes("Math.floor(Math.max(0, save.towerFloor || 1) / 10)") && shellUi.includes("tower floor ${nextLevel}") && relics.includes("Math.min(5"));
