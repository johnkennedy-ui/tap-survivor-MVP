...
check("level-up skill effects animate from split sprites", levelUp.includes("createChoiceIcon") && levelUp.includes("animateChoiceSprite") && levelUp.includes("trimmedFrameBounds") && levelUp.includes("iconScale") && Object.values(content.assets?.sprites?.weapons || {}).every((sprite) => sprite?.src?.includes("skill-effects/split") ? Number.isFinite(sprite.iconScale) : true) && (content.runUpgrades || []).every((upgrade) => Number.isFinite(content.assets?.sprites?.runUpgrades?.[upgrade.id]?.iconScale)));
